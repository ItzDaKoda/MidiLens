...
export default function PianoRoll({ analysis }) {
  ...
  function setupSynthAudio() {
    ...
    synthRef.current = new Tone.PolySynth(Tone.Synth, synthOptions).connect(
      ...
      while (
        ...
        if (note.start + note.duration >= startFrom) {
          ...
          if (playableDuration > 0) {
            scheduleNote(note, transportTime, playableDuration);
          }
        }
  ...
  if (!notes.length) {
    return <div className="piano-empty">No notes found in this file.</div>;
  }
  ...
  return (
    <div className="piano-shell">
      ...
      <div className="control-row">
        <label>
          Sound
          <select value={soundPreset} onChange={(e) => setSoundPreset(e.target.value)}>
        ...
        <label className="toggle-line">
          <input
            type="checkbox"
            checked={soloMelody}
            onChange={(e) => setSoloMelody(e.target.checked)}
          />{" "}
          Skip drum channel
        </label>
      </div>
      ...
      <div className="piano-roll-scroll">
        <div
          ...
          style={{ width: `${rollWidth}px`, height: `${rollHeight}px` }}
        >
          {beatGrid.map((beat) => (
            <div
          ...
          {visualNotes.map((note, index) => {
            ...
            const isActive =
              playheadTime >= note.start && playheadTime <= note.start + note.duration;

            return (
              <div
                key={`${note.pitch}-${note.start}-${index}`}
                className={`piano-note ${note.isDrum ? "drum" : ""} ${
                  isActive ? "active" : ""
                }`}
                style={{
                  left: `${left}px`,
