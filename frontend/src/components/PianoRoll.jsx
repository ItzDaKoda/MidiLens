...
export default function PianoRoll({ analysis }) {
  ...
  function setupSynthAudio() {
    ...
    if (soundPreset === "soft") {
      ...
      synthOptions = {
        ...
        envelope: { attack: 0.005, decay: 0.12, sustain: 0.35, release: 0.3 },
      };
    } else {
      synthOptions = {
        oscillator: { type: "triangle" },
        envelope: { attack: 0.008, decay: 0.16, sustain: 0.45, release: 0.45 },
      };
    }

    synthRef.current = new Tone.PolySynth(Tone.Synth, synthOptions).connect(compressorRef.current);
    ...
    synthRef.current.volume.value = Tone.gainToDb(Math.max(volume, 0.01));
    drumSynthRef.current.volume.value = Tone.gainToDb(Math.max(volume * 0.45, 0.01));
  }
  ...
  useEffect(() => {
    if (synthRef.current) synthRef.current.volume.value = Tone.gainToDb(Math.max(volume, 0.01));
    if (drumSynthRef.current) drumSynthRef.current.volume.value = Tone.gainToDb(Math.max(volume * 0.45, 0.01));
  }, [volume]);
  ...
  function scheduleNote(note, transportTime, playableDuration) {
    if (note.isDrum && soloMelody) return;

    const midiNote = Tone.Frequency(note.pitch, "midi").toNote();
    const velocity = Math.min(1, Math.max(0.08, note.velocity / 127));
    const safeDuration = Math.max(playableDuration, 0.05);

    Tone.Transport.schedule((time) => {
      if (note.isDrum && drumSynthRef.current) {
        drumSynthRef.current.triggerAttackRelease("16n", time, velocity);
      } else if (synthRef.current) {
        synthRef.current.triggerAttackRelease(midiNote, safeDuration, time, velocity);
      }
    }, transportTime);
  }
  ...
  async function playPlayback() {
    ...
    playbackStartRef.current = Tone.now();

    nextNoteIndexRef.current = playableNotes.findIndex(
      (note) => note.start + note.duration >= startFrom
    );
    ...
    const scheduler = () => {
      ...
      const currentSongTime = startFrom + elapsed;
      const scheduleUntil = currentSongTime + lookAheadSeconds;

      while (
        nextNoteIndexRef.current < playableNotes.length &&
        playableNotes[nextNoteIndexRef.current].start <= scheduleUntil
      ) {
        const note = playableNotes[nextNoteIndexRef.current];

        if (note.start + note.duration >= startFrom) {
          const transportTime = Math.max(0, note.start - startFrom);
          const playableDuration =
            note.start < startFrom
              ? note.duration - (startFrom - note.start)
              : note.duration;

          if (playableDuration > 0) scheduleNote(note, transportTime, playableDuration);
        }
    ...
    scheduler();
    schedulerRef.current = setInterval(scheduler, 120);
    ...
    <div className="piano-shell">
      <div className="transport-bar">
        <div className="transport-left">
          ...
          <button className="ghost-control" onClick={() => stopPlayback(true)}>
            <Square size={16} /> Stop
          </button>

          <button className="ghost-control" onClick={() => setPlayheadTime(0)}>
      ...
      <input
        ...
        step="0.01"
        value={playheadTime}
        onChange={(event) => setPlayheadTime(Number(event.target.value))}
      />
      ...
      <div className="control-row">
        <label>
          ...
          <select value={soundPreset} onChange={(event) => setSoundPreset(event.target.value)}>
            ...
            <option value="bright">8-Bit Bright</option>
          </select>
        </label>
        ...
        <label className="toggle-line">
          <input
            type="checkbox"
            checked={soloMelody}
            onChange={(event) => setSoloMelody(event.target.checked)}
          /> Skip drum channel
        </label>
      </div>
      ...
      <div className="piano-roll-scroll">
        <div className="piano-roll-canvas" style={{ width: `${rollWidth}px`, height: `${rollHeight}px` }}>
          {beatGrid.map((beat) => (
            <div
              key={`${beat.seconds}-${beat.beat}`}
              className={beat.isMeasure ? "measure-line" : "beat-line"}
              style={{ left: `${beat.seconds * zoom}px` }}
            />
          ))}
